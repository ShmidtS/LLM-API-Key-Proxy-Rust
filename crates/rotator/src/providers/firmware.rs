...
impl FirmwareProvider {
    pub fn new() -> Self {
        Self {
            base_url: "https://app.firmware.ai/api/v1".to_owned(),
        }
    }
}

#[async_trait]
impl Provider for FirmwareProvider {
    fn id(&self) -> &str {
        "firmware"
    }
...
mod tests {
    ...
    #[test]
    fn firmware_provider_exposes_expected_metadata_and_auth() {
        let provider = FirmwareProvider::new();

        assert_eq!(provider.id(), "firmware");
        assert_eq!(provider.base_url(), "https://app.firmware.ai/api/v1");
        assert_eq!(
