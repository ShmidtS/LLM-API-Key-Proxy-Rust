...
impl IflowProvider {
    pub fn new() -> Self {
        Self {
            base_url: "https://apis.iflow.cn/v1".to_owned(),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn iflow_provider_exposes_expected_metadata_and_auth() {
        let provider = IflowProvider::new();

        assert_eq!(provider.id(), "iflow");
        assert_eq!(provider.base_url(), "https://apis.iflow.cn/v1");
        assert_eq!(
