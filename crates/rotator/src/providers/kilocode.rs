...
impl KiloCodeProvider {
    pub fn new() -> Self {
        Self {
            base_url: "https://kilo.ai/api/openrouter".to_owned(),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn kilocode_provider_exposes_expected_metadata_and_auth() {
        let provider = KiloCodeProvider::new();

        assert_eq!(provider.id(), "kilocode");
        assert_eq!(provider.base_url(), "https://kilo.ai/api/openrouter");
        assert_eq!(
